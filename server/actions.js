...
function executeBuild(gameState, playerId, params) {
  ...
  if (slot.built) {
    // Overbuilding rules: can overbuild own lower-level tile
  ...
  const coalFlip = applyConsumption(gameState, coalResult.sources);
  const ironFlip = applyConsumption(gameState, ironResult.sources);

  // Handle income from flipped resource tiles
  if (coalFlip.flipped && coalFlip.owner) {
    const owner = gameState.players[coalFlip.owner];
    if (owner) {
      owner.trackPos = Math.min(owner.trackPos + coalFlip.income, INCOME_TRACK.length - 1);
    }
  }
  if (ironFlip.flipped && ironFlip.owner) {
    const owner = gameState.players[ironFlip.owner];
    if (owner) {
      owner.trackPos = Math.min(owner.trackPos + ironFlip.income, INCOME_TRACK.length - 1);
    }
  }
...
function executeNetwork(gameState, playerId, params) {
  ...
  if (coalNeeded > 0) {
    const firstLink = linkPlacements[0];
    const coalResult = consumeCoal(gameState, firstLink.from, coalNeeded, playerId);
    if (!coalResult.success) return coalResult;
  ...
  if (beerNeeded > 0) {
    const secondLink = linkPlacements[1] || linkPlacements[0];
    const beerResult = findBeer(gameState, secondLink.to, beerNeeded, playerId, null);
    if (!beerResult.success) return { success: false, reason: '建造雙鐵路需要 1 桶啤酒，但找不到可用的啤酒廠' };

    const beerFlip = applyConsumption(gameState, beerResult.sources);
    if (beerFlip.flipped && beerFlip.owner) {
      const owner = gameState.players[beerFlip.owner];
      if (owner) owner.trackPos = Math.min(owner.trackPos + beerFlip.income, INCOME_TRACK.length - 1);
    }
  }
  ...
  // 應用煤炭消耗
  if (coalSources) {
    const flip = applyConsumption(gameState, coalSources);
    if (flip.flipped && flip.owner) {
      const owner = gameState.players[flip.owner];
      if (owner) owner.trackPos = Math.min(owner.trackPos + flip.income, INCOME_TRACK.length - 1);
    }
  }
...
function executeDevelop(gameState, playerId, params) {
  ...
  const flip = applyConsumption(gameState, ironResult.sources);
  if (flip.flipped && flip.owner) {
    const owner = gameState.players[flip.owner];
    if (owner) {
...
function findReachableMerchants(gameState, locationId, industryType) {
  ...
  for (const merchant of merchantList) {
    ...
    if (!merchant.accepts.includes(industryType)) continue;
...
function executeSell(gameState, playerId, params) {
  ...
  if (totalBeer > 0) {
    ...
    const beerResult = findBeer(gameState, firstSale.locationId, totalBeer, playerId, merchantId);
    if (!beerResult.success) return beerResult;
    ...
    if (beerFlip.flipped && beerFlip.owner) {
      const owner = gameState.players[beerFlip.owner];
      if (owner) {
        owner.trackPos = Math.min(owner.trackPos + beerFlip.income, INCOME_TRACK.length - 1);
      }
    }
  }
  ...
  if (usedMerchantBeer && usedMerchantBeer.bonusType) {
    ...
    switch (m.bonusType) {
      ...
      case 'income':
        // Oxford / Nottingham: 收入軌前進 2 格
        player.trackPos = Math.min(player.trackPos + m.bonusAmount, INCOME_TRACK.length - 1);
        bonusMsg = `（${m.name} 獎勵：收入軌 +${m.bonusAmount} 格）`;
        break;

      case 'develop':
        // Gloucester / Warrington: 免費研發 1 個（不用鐵）
        // 標記待處理，讓前端彈窗給玩家選
        bonusMsg = `（${m.name} 獎勵：免費研發 1 個 — 請選擇）`;
        // 設定 pendingBonus 讓前端知道要彈窗
...
function executeLoan(gameState, playerId, params) {
  ...
  // 借貸：收入等級退3級，停在新等級最高格
  const oldLevel = getIncomeLevel(player.trackPos);
  const newPos = loanDecreasePosition(player.trackPos);

  if (newPos < 0) {
    return { success: false, reason: `無法借貸！收入等級 ${oldLevel} 退3級會低於 -10` };
  }

  const newLevel = getIncomeLevel(newPos);
  ...
  return {
    success: true,
    message: `貸款：+£${LOAN_AMOUNT}，收入 ${oldLevel} → ${newLevel}（退3級，格${player.trackPos}）`
  };
}
